...
Adjusting raw counts of data in order to make them comparable relative to one another; often involves turning counts into a percentage or ratio
...
leaflet() %>%
  setView(lat = 42.4, lng = -70, zoom = 6) %>%
...
```{r out.width= 500}
knitr::include_graphics("img/class1.png")
```
...
```{r out.width= 500}
knitr::include_graphics("img/class2.png")
```
:::
:::
...
leaflet("width=90%, height=70%") %>%
  setView(lat = 42.4, lng = -70, zoom = 9) %>%
  addProviderTiles("CartoDB.Positron") %>%
  addPolygons(
...
```{r out.width= 500}
knitr::include_graphics("img/agg1.png")
```
...
```{r out.width= 500}
knitr::include_graphics("img/agg2.png")
```
:::
:::
